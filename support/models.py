...
class Conversation(models.Model):
  ...
  def __str__(self):
    return f"Concersation #{self.id} - {self.user.username} Order #{self.order.id}"
...
class Message(models.Model):
  ...
  def __str__(self):
    return f"{self.role} : {self.content[0:50]}" # Agent : we are checking your request : Please wait ....
